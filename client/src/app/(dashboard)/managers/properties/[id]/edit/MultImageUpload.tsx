...
export const MultiImageUpload: React.FC<MultiImageUploadProps> = ({
    ...
    rejectedFiles.forEach(rejection => {
      rejection.errors.forEach((error: any) => {
        if (error.code === 'file-too-large') {
          fileErrors.push(`${rejection.file.name} is larger than ${maxSizeInMB}MB`);
        } else if (error.code === 'file-invalid-type') {
          fileErrors.push(`${rejection.file.name} is not a supported image type`);
        } else {
  ...
  return (
    <div className="space-y-4">
      ...
      {files.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-lg font-medium">New Photos to Upload ({files.length})</h3>
